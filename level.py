	...
	def setup_level(self):
		for row_index,row in enumerate(LEVEL_MAP):
			for col_index,col in enumerate(row):
				x = col_index * sizes["TILE_SIZE"]
				y = row_index * sizes["TILE_SIZE"]
				if col == 'X':
					Tile((x,y),[self.visible_sprites,self.collision_sprites])
				if col == 'P':
					self.player = Player((x,y),[self.visible_sprites,self.active_sprites],self.collision_sprites)
				if col == 'L':
					Palm((x,y),[self.visible_sprites])
				if col == 'C':
					Cloud((x,y),[self.visible_sprites,self.active_sprites])
				if col == 'W':
					FakeTile((x,y),[self.visible_sprites])
				if col == 'E':
